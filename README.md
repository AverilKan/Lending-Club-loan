...
├── 2_Modelling.ipynb                # Model training & evaluation (generates artifact)
├── 3_Deployment.ipynb               # Inference demo
├── src/
│   ├── transformers.py              # Custom sklearn transformers
│   └── predictor.py                 # CreditRiskPredictor class
├── models/
│   └── [credit_risk_pipeline_investor.joblib generated by 2_Modelling.ipynb]
├── data/
...
Notebooks are saved with outputs — you can review results and approach without re-running.

**To explore the analysis and generate the model:**
...
# Run 1_EDA.ipynb and 2_Modelling.ipynb sequentially to train and save the model
# Then run 3_Deployment.ipynb to make predictions
```

**Note:** The `models/credit_risk_pipeline_investor.joblib` artifact is generated by running `2_Modelling.ipynb`. It's not included in the repository to keep it lightweight—regenerate it from the notebook if needed.
...
After generating the model artifact by running `2_Modelling.ipynb`:
